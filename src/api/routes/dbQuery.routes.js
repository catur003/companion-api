...
// POST /db/query   body: { databaseUuid, sql }
router.post('/db/query', async (req, res) => {
  const { databaseUuid, sql } = req.body || {};
  if (!databaseUuid || !sql) {
  ...
  try {
    const result = await dbBrowser.runSelectQuery(databaseUuid, sql);
    audit.record({ action, databaseUuid, sql, ok: true, auditLevel: policy.auditLevel });
    return res.json({ success: true, message: 'OK', code: 'OK', data: result });
  } catch (err) {
    audit.record({ action, databaseUuid, sql, ok: false, error: err.message });
